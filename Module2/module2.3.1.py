...
PARENT_PATH = os.path.join(os.path.dirname(__file__), 'resonance')
print(PARENT_PATH)
expname = 'resonance'
...
f_array = []
V_DUT = []
def Input_output(frequency, amplitude):
    Resistance = 220
    Inductance = 0.1
    ...
    tutorial_df = pd.DataFrame({'time' : t, 'V1' : V1_np, 'V2' : V2_np, 'sineoutput_freq' : frequency, 'sineoutput_amplitude':amplitude, 'Resistance':Resistance, 'Inductance': Inductance, 'Capacitance':capacitance})
    tutorial_df.to_csv(os.path.join(PARENT_PATH, expname+'freq'+str(frequency)+'amp'+str(amplitude)+'.csv'))
    V_DUT.append(np.max(V1_np))
