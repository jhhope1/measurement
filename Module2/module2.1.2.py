...
set_square_output(frequency, amplitude)
...
tutorial_df = pd.DataFrame({'time' : t, 'V1' : V1_np, 'V2' : V2_np, 'sineoutput_freq' : frequency, 'sineoutput_amplitude':amplitude, 'Resistance':Resistance, 'Inductance': Inductance})
tutorial_df.to_csv(os.path.join(PARENT_PATH, expname+'freq'+str(frequency)+'amp'+str(amplitude)+'.csv'))

plt.plot(numpy.fromiter(rg1, dtype = numpy.float))
plt.plot(numpy.fromiter(rg2, dtype = numpy.float))
plt.show()
