...
plt.rc('font', size = 13)
plt.title('Curve between $\mathrm{t}$ and $\mathrm{V_{C}}$')
plt.xlabel('$t$[$ms$]')
plt.ylabel('$V$[V]')
...
plt.legend(loc=0)
plt.savefig(os.path.join(save_path,'RC_V-t_curve.pdf'))
plt.show()
...
plt.title('Correlation between $t$ and $log(V_{in}-V_{C})$')
plt.xlabel('$t$[$ms$]')
plt.ylabel('$log(V_{in}-V_{C})[-]$')
plt.xlim(x_llim, x_rlim)
plt.ylim(y_dlim, y_ulim)
...
plt.legend(loc=0)
plt.savefig(os.path.join(save_path,'RC_log(V-V0)-t_correlation.pdf'))
plt.show()
