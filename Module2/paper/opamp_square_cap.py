...
data = pd.read_csv(data_path+"\\Module2square_2kHz_capa양단.csv", skiprows=28)
...
plt.legend(loc=0)
plt.savefig(os.path.join(save_path,'opamp_integrate_sqaure_cap.pdf'))
plt.show()
...
plt.legend(loc=0)
plt.savefig(os.path.join(save_path,'opamp_original_sqaure_cap.pdf'))
plt.show()