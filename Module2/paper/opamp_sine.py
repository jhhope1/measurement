...
data = pd.read_csv(data_path+"\\Module2sine_2kHz.csv", skiprows=28)
...
plt.plot(x, integrate,'.', label='$\int -V/RC dt$', markersize = 2, color = 'blue')
plt.legend(loc=0)
plt.savefig(os.path.join(save_path,'opamp_integrate_sine.pdf'))
plt.show()
...
plt.legend(loc=0)
plt.savefig(os.path.join(save_path,'opamp_original_sine.pdf'))
plt.show()