...
data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data\\exp3\\3_Module2sine_1kHz_1kOhm_OP97F.csv')

#data = dictionary format data from WaveForms exported raw data
data = csv_data(data_path)
X = [data['time'], data['time']]
Y = [data['V1'], data['V2']]
Plot(X, Y , X_unit = 's', Y_unit = 'V', X_name = '$t$', Y_name = '$V_{in}, V_{out}$', graph_name = 'exp3_3_buffer_1kOhm', save_path = save_path, linear_fit = False, File_format = 'jpg', labels = ('$V_{in}$','$V_{out}$'))
