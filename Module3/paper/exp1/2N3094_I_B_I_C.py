...
data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data\\exp1\\2N3904\\exp_sameasabove_IB_IC.csv')

#data = dictionary format data from WaveForms exported raw data
data = csv_data(data_path)
X = data['V1']/1e5
Y = data['V2']/100
Plot(X, Y , X_unit = 'A', Y_unit = 'A', X_name = '$I_{B}$', Y_name = '$I_{C}$', graph_name = '2N3904_IB_IC', save_path = save_path, linear_fit = True, File_format = 'jpg')
