...
data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data\\exp2\\exp2.1\\Vin_VCE_R_adjustable_changed_2.csv')

#data = dictionary format data from WaveForms exported raw data
data = csv_data(data_path, [0, 3000])
X = [data['time'], data['time']]
Y = [data['V1'], data['V2']]
Plot(X, Y , X_unit = 's', Y_unit = 'V', X_name = '$t$', Y_name = '$V_{in}, V_{CE}$', graph_name = 'CEA_V_in_V_CE_2', save_path = save_path, linear_fit = False, File_format = 'jpg', labels = ('$V_{in}$','$V_{CE}$'))
